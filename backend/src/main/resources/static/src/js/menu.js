...
// Inicializa: obtiene datos del API (Spring Boot) y pinta
async function init() {
  try {
    const list = await foodController.listFoods();
    state.items = list.map(toItem).filter(Boolean);
  } catch (err) {
    state.items = [];
    const container = document.getElementById('menu-cards');
    if (container) container.innerHTML = '<p class="menu-error">No se pudo cargar el menú. Comprueba que el servidor esté en marcha.</p>';
  }
  renderPills();
  render();
  bindToggle();
